...
hyper_param['p2'] = 0.5
for hyper_param['p1'] in [0.5, 0.9]:
    for hyper_param['ntimes_ns'] in [2]:#[1.5,2,2.5,3,3.5,4]:
        for SNR in np.arange(0,-18,-2):    #([np.arange(100,11,-5), np.arange(10,0,-1)]):
            ...
            hyper_param_string = 'Results_C111423_p1_' + str(hyper_param['p1']) +"_nstimes_" + str(hyper_param['ntimes_ns'])#'s1_' + str(hyper_param['s1']) + 's2_' + str(hyper_param['s2'])
                                 #+ 'p_' + str(hyper_param['p1']) + "_Errthresh_2"

            results_folder = results_folder_top + 'ResultsBackup' + '/' + hyper_param_string  # 'p1_'+str(hyper_param['p1']) + '_p2_'+str(hyper_param['p2'])
            try:
                os.mkdir(results_folder)
